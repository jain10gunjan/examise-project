...
export default function Sidebar() {
  ...
  const getRandomNumberInRange1 = (min1: number, max1: number): number => {
    return Math.floor(Math.random() * min1 - (max1 + 1)) + min1;
  };

  const getRandomNumberInRange2 = (min2: number, max2: number): number => {
    return Math.floor(Math.random() * min2 - (max2 + 1)) + max2;
  };

  const randomNum1 = getRandomNumberInRange1(min1, max1);
  const randomNum2 = getRandomNumberInRange2(min2, max2);
      ...
      <div className="lg:col-span-1 lg:w-full lg:h-full lg:bg-gradient-to-r lg:from-gray-50 lg:via-transparent lg:to-transparent dark:from-neutral-800">
        <div className="sticky top-0 start-0 py-8 lg:ps-8">
          ...
          <div className="space-y-6">
            {JSON_CALCULATORS.slice(0, 8).map((data, index) => (
              <a
                key={index}
              ...
              >
                <div className="grow">
                  <p className="text-sm font-bold text-gray-800 group-hover:text-blue-600 dark:text-neutral-200 dark:group-hover:text-blue-500">
                    {data.title}
                  </p>
                  <span className="text-sm text-gray-600 group-hover:text-blue-600 dark:text-neutral-200 dark:group-hover:text-blue-500">
                    {data.description}
                  </span>
                </div>
