...
export default function Question() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        const res = await fetch(`https://us-east-1.aws.data.mongodb-api.com/app/aptitude_tracker_api-fjroz/endpoint/aptitudeData?_id=${qid.id}`);
        if (!res.ok) {
    ...
    "hasPart": [
      {
        ...
        "text":
          `${data[0]?.question.replace(/<[^>]*>?/gm, '')}`,
        "acceptedAnswer": {
          "@type": "Answer",
          "text": `Correct option is : ${data[0]?.correct_option.replace(/<[^>]*>?/gm, '')} \n ${data[0]?.solution.replace(/<[^>]*>?/gm, '')}`
        }
      }
    ]
  }];
              ...
              <MathJaxContext>
                <div>
                  <div className="relative question-numbercontainer">
                    <p className="text-xs text-gray-600">Aptitude Questions <br />Chapter : {data[0]?.topic} </p>
                    <p className="text-xs text-gray-600">Difficulty  : {data[0]?.difficulty} </p>
