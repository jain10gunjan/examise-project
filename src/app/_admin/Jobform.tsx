...
function JobsForm() {
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      const response = await axios.get('https://unstop.com/api/workrelationship/workfunction/getAll');
  ...
  const fetchDataUnstop = async (name:string) => {
    try {
      ...
      const response = await axios.get(`https://unstop.com/api/public/opportunity/search-result?opportunity=jobs&page=1&per_page=15&oppstatus=open&category=${name}`);
      const newJobs = response?.data?.data?.data;

      if(newJobs){
        // Process and post new data
      const transformedJobs = await Promise.all(newJobs.map(async (job:any) => {
        try {
          const searchIdResponse = await axios.get(`https://us-east-1.aws.data.mongodb-api.com/app/aptitude_tracker_api-fjroz/endpoint/searchIdFromSearchId?searchIds=${job?.id}`);
          const exists = searchIdResponse.data.status === 'exists';

          if (!exists) {
            const competitionResponse = await axios.get(`https://unstop.com/api/public/competition/${job?.id}`);
            return {
              ...
              category:name,
              salary: job?.jobDetail?.not_disclosed ? false : "Not Disclosed",
              type: job?.jobDetail?.type === "in_office" ? "In Office" : null,
              applyBefore: new Date(job?.regnRequirements?.end_regn_dt).toLocaleDateString('en-US', { day: 'numeric', month: 'short', year: 'numeric' }),
              requirements: null,
              link: competitionResponse?.data?.data?.competition?.regn_url === null ? job?.seo_url : competitionResponse?.data?.data?.competition?.regn_url,
              image: job?.logoUrl2,
              createdat:new Date().toDateString(),
              description: competitionResponse?.data?.data?.competition?.details
            };
          } else {
            return null;
          }
      ...
      const filteredTransformedJobs = transformedJobs.filter(job => job !== null);
      ...
      const postDataResponse = await axios.post('https://us-east-1.aws.data.mongodb-api.com/app/aptitude_tracker_api-fjroz/endpoint/jobsform', { data: filteredTransformedJobs });
      toast.success(filteredTransformedJobs.length + "Jobs Posted Successfully");
                  ...
                  {categories.map((name:any,index) => {
                    return <tr key={index}>
                      ...
                      <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 ">
                        <button onClick={()=>{fetchDataUnstop(name?.name)}}>Fetch Data</button>
                      </td>
                    </tr>
