...
const MCQComponent: React.FC = () => {
  ...
  let quizid = useParams<{ id: string }>();
    ...
    async function fetchData() {
      ...
      try {
        const res = await fetch(
          `https://us-east-1.aws.data.mongodb-api.com/app/aptitude_tracker_api-fjroz/endpoint/currentaffairsquiz?_id=${quizid.id}`
        );
        if (!res.ok) {
  ...
  const dateParts = quiz?.topic.split("-").slice(-3);
          ...
          <div className="container mx-auto flex flex-col px-5 py-4 justify-center items-center">
            <div className="w-full md:w-2/3 flex flex-col mb-16">
              <h1 className="text-3xl text-gray-800 font-bold md:text-4xl md:leading-tight lg:text-5xl lg:leading-tight dark:text-neutral-200">
                Practice quiz for Current Affairs{" "}
            ...
            {quiz?.questions?.map((data: Question, index: number) => (
              <div key={index} className="w-full  flex flex-col mb-16r">
                  ...
                  <div className="flex border-b border-gray-200 justify-center">
                    <MathJaxContext>
                      <div className="w-full md:w-2/3">
                        ...
                        <div className="questioncontainer my-4">
                        ...
                        <div className="flex-col leading-none optionscontainer">
                          {Object.keys(data.options).map((optionKey) => (
                            <MathJax inline dynamic key={optionKey}>
                              <div
                                onClick={() =>
                                  handleOptionClick(
                                    optionKey,
                                ...
                                className={`cursor-pointer p-2 my-1 rounded ${
                                  selectedOption[index] === optionKey
                                    ? optionKey === data.correct_option
                                      ? "bg-green-200"
                                      : "bg-red-200"
                                    : "bg-gray-200"
                                }`}
                                dangerouslySetInnerHTML={{
                                  __html: data?.options[optionKey],
                                }}
                              />
                            </MathJax>
                          ))}
