...
const calculateAnnealingTemp = (primerSeq: string): number => {
  const atCount = (primerSeq.match(/[AT]/gi) || []).length;
  const gcCount = (primerSeq.match(/[GC]/gi) || []).length;
  return (2 * atCount) + (4 * gcCount);
};

const convertToFahrenheit = (celsius: number): number => (celsius * 9 / 5) + 32;

const AnnealingTempCalculator: React.FC = () => {
  const { register, handleSubmit } = useForm<FormValues>();
  const [result, setResult] = useState<Result | null>(null);
  const [unit, setUnit] = useState<'C' | 'F'>('C');

  const onSubmit: SubmitHandler<FormValues> = (data) => {
    const annealingTempCelsius = calculateAnnealingTemp(data.primerSeq);
    setResult({
      celsius: annealingTempCelsius,
      fahrenheit: convertToFahrenheit(annealingTempCelsius),
    });
  };
...
export default AnnealingTempCalculator;
